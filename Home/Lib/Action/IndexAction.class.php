...
class IndexAction extends CommonAction {
	...
	private function mobile($name){
		...
		$TagsType = M('TagsType');
		if($name=='index'){
			$list = $Model->field('id','title','url','icon','logo','description')->where('status=1')->order('sort_order desc,id asc')->limit("{$page},{$perpage}")->select();
		}else{
	...
	public function search(){
		...
		$kw = trim($_GET['kw']);
		$where = "status=1";
		if(!empty($kw)) $where .=" AND `title` LIKE '%{$kw}%' OR `host` LIKE '%{$kw}%'  ";
